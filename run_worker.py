...
def git(command, repo_path, check=True):
...
def current_branch(repo_path):
    return git(["git", "branch", "--show-current"], repo_path).stdout.strip()


def git_clean(repo_path):
    return git(["git", "status", "--porcelain"], repo_path).stdout.strip() == ""


def remote_origin(repo_path):
    return git(["git", "remote", "get-url", "origin"], repo_path).stdout.strip()


def gh_auth_ok(repo_path):
    return git(["gh", "auth", "status"], repo_path, check=False).returncode == 0
...
def preflight(repo_path):
    ...
    if not remote_origin(repo_path):
        raise GitHubWorkerError("Refusing to work: no remote origin configured")


def branch_exists(repo_path, branch):
    local = git(["git", "show-ref", "--verify", f"refs/heads/{branch}"], repo_path, check=False)
    if local.returncode == 0:
        return True
    remote = git(["git", "ls-remote", "--exit-code", "--heads", "origin", branch], repo_path, check=False)
...
def checkout_new_branch(repo_path, branch):
    git(["git", "checkout", "-b", branch], repo_path)


def cleanup_dirty(repo_path):
    status = git(["git", "status", "--porcelain"], repo_path, check=False).stdout
    if not status.strip():
        return
    git(["git", "restore", "--staged", "."], repo_path, check=False)
    tracked = []
    untracked = []
    for line in status.splitlines():
        ...
        if line.startswith("?? "):
            ...
            tracked.append(path)
    if tracked:
        git(["git", "restore", "--worktree", *tracked], repo_path, check=False)
...
def checkout_main(repo_path):
    git(["git", "checkout", "main"], repo_path, check=False)


def commit_all(repo_path, message):
    git(["git", "add", "."], repo_path)
    git(["git", "commit", "-m", message], repo_path)


def execute_real_code_task(task, repo_path):
    try:
        from agent_executor import execute_task as agent_execute_task
        return agent_execute_task(task, repo_path)
    except ModuleNotFoundError:
        path = os.path.join("backend", f"{safe_branch_name(task).split('/')[-1]}.py")
        os.makedirs(os.path.join(repo_path, "backend"), exist_ok=True)
        with open(os.path.join(repo_path, path), "w", encoding="utf-8") as file:
...
def run_one(project_path, repo_path):
    ...
    try:
        ...
        files = execute_real_code_task(task, repo_path)
        print(f"Files changed: {', '.join(files)}")
        if remote_branch_exists(repo_path, branch):
            branch = unique_branch(repo_path, safe_branch_name(task))
            print(f"Remote branch exists before push; switching to {branch}")
            checkout_new_branch(repo_path, branch)
            task = mark_task_running(task_file, branch)

        commit_all(repo_path, f"AI task: {task['title']}")
        git(["git", "push", "-u", "origin", branch], repo_path)
        pr_url = git([
        ...
        commit_and_push_status_update(repo_path, task, branch)
        ...
        if running:
            ...
            try:
                ...
                if branch:
                    git(["git", "push", "-u", "origin", branch], repo_path, check=False)
            except Exception:
                pass
